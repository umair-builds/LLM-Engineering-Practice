...
def run_answer_evaluation(selected_test_index, progress=gr.Progress()):
    """Run answer evaluation for the selected test case (Groq: one test to avoid rate limit)."""
    all_feedbacks = []
    total_accuracy = 0.0
    ...
    for test, result, prog_value in evaluate_all_answers(test_indices=[selected_test_index]):
        count += 1
        ...
        # Save feedback
        all_feedbacks.append(result.feedback)
    ...
    # Combine feedback into HTML
    feedback_html = "<br>".join([f"<p style='margin-bottom:10px;'>{fb}</p>" for fb in all_feedbacks])
    ...
    <div style="padding: 0;">
        ...
        <div style="margin-top: 20px; padding: 15px; background-color: #f0f0f0; border-radius: 8px;">
            <h4 style="margin-bottom: 10px;">Judge Feedback:</h4>
            {feedback_html}
        </div>
        <div style="margin-top: 20px; padding: 10px; background-color: #d4edda; border-radius: 5px; text-align: center; border: 1px solid #c3e6cb;">
            <span style="font-size: 14px; color: #155724; font-weight: bold;">✓ Evaluation Complete: {count} tests</span>
...
def main():
    ...
    with gr.Blocks(title="RAG Evaluation Dashboard", theme=theme) as app:
        ...
        tests_for_choices = load_tests()
        answer_test_choices = [
            (f"Test {i}: {t.question[:55]}..." if len(t.question) > 55 else f"Test {i}: {t.question}", i)
            for i, t in enumerate(tests_for_choices)
        ]
        answer_test_dropdown = gr.Dropdown(
            choices=answer_test_choices,
            value=0,
            label="Choose test case (one test = fewer Groq rate limits)",
            type="index",
        )
